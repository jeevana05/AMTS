...
const BookingKDPtoJMD = () => {
    ...
    useEffect(() => {
        const fetchSchedule = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:5000/schedule/booking?from_station=Kadapa&to_station=Jammalamadugu`);
                ...
                if (data.length > 0) {
                    setTicketPrice(150); // Set default ticket price, adjust as needed
                }
            } catch (error) {
    ...
    return (
        <div className={styles.pageContainer}>
            <h2 className={styles.header}>Booking Page - KDP to JMD</h2>
...
export default BookingKDPtoJMD;
